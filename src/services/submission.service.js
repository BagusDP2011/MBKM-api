...
async function submit(submission) {
  const firstApprover = await submissionRepo.getFirstApproverByProdiId(
    submission.ProdiID
  );
  if (!firstApprover)
    throw new Error("No Approver on this prodi, please set level approver");

  const submissionId = uuidv4();
  await submissionRepo.create(submissionId, submission);
  if (submission.ProgramType == "Pertukaran Pelajar") {
    const exchangeId = uuidv4();
    await exchangeProgramRepo.addExchangeProgram(
      exchangeId,
      submissionId,
      submission.ExchangeProgram
    );
  ...
  return await submissionRepo.createSubmissionApproval(
    submissionId,
...
async function approve(submissionId, accessId) {
  let currApprover = await submissionRepo.getCurrentApprover(
    submissionId,
    accessId
  );
  if (!currApprover)
    throw new Error("You dont have access to approve this submission");

  await Promise.all([
    submissionRepo.updateSubmissionApproval(
      submissionId,
      currApprover.ApproverID,
      "",
      "Approved"
    ),
  ]);

  let nextApprover = await submissionRepo.getNextApprover(
    currApprover.ProdiID,
    currApprover.Level + 1
  );

  if (nextApprover) {
    await submissionRepo.createSubmissionApproval(
      submissionId,
      nextApprover.ApproverID,
      "Pending"
    );
  } else {
    let submission = await submissionRepo.getSubmissionById(
      submissionId,
      accessId
    );
...
async function reject(submissionId, accessId, body) {
  ...
  if (!currApprover)
    throw new Error("You dont have access to reject this submission");
...
async function getSubmissionsById(id, accessID) {
  let submissions = await submissionRepo.getSubmissionByUserId(id, accessID);
...
async function getSubmissionDetail(submissionId, user) {
  let [submission, subApproval, subAttachment, exchangeProgram] =
    await Promise.all([
  ...
  if (exchangeProgram) {
    const courses = await exchangeProgramRepo.getCoursesByExchangeID(
      exchangeProgram.ExchangeID
    );
...
async function getSubmissionByAccessID(accessId, req) {
  ...
  const submissions = await submissionRepo.getSubmissionByAccessID(
    accessId,
    user.id
  );
  submissions.forEach((submission) => {
  ...
  return await Promise.all(
    submissions.map(async (item) => {
      let submissionApproval =
        await submissionRepo.getSubmissionApprovalBySubmission(
          item.SubmissionID
        );
      return {
        ...item,
...
async function updateLucturerSubmission(body) {
  await submissionRepo.updateLucturerSubmission(
    body.SubmissionID,
    body.LecturerGuardianID
  );
}
